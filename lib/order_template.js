...
module.exports = {
  ...
  cart: function (price, recipe, temp_recipe, cartList, request, login = `<a id="visit" href="/cus/login">로그인 | </a> <a id="visit" href="/cus/sign_up">회원가입</a>`, admin = '') {
    cartList += `</table><input type='hidden' name='id' value=${temp_recipe.temp_recipe_id}><input type='submit' value='삭제하기'></form>`
    recipe += `<input type='submit' value='주문하기'></form>`
      var body = `
      ${login}
      ${admin}
